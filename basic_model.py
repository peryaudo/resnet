...
class BasicResNetLayer(nn.Sequential):
...
class BasicResNetModel(nn.Module):
    def __init__(self, num_channels=[64, 128, 256, 512], num_blocks=[2, 2, 2, 2], num_classes=10):
        ...
        layers = [BasicResNetLayer(num_channels[0], num_channels[0], num_blocks[0], stride=1)]
        for i in range(1, 4):
            layers.append(BasicResNetLayer(num_channels[i - 1], num_channels[i], num_blocks[i], stride=2))
...
def resnet18():
    return BasicResNetModel(num_blocks=[2, 2, 2, 2])

def resnet34():
    return BasicResNetModel(num_blocks=[3, 4, 6, 3])